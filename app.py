...
import sensor_realism
import xai_generation
from png_metadata_parser import PNGMetadataParser


APP_TITLE = "Poopster PNG Metadata Viewer 💩"
...
XAI_OUTPUT_DIR = Path(os.getenv("XAI_OUTPUT_DIR", "/cache/xai-output")).resolve()
...
def upload_too_large(e):
    return {"error": f"Upload too large. Max is {MAX_UPLOAD_MB} MB."}, 413
...
xai_video_jobs_lock = threading.Lock()
xai_video_jobs = {}
...
def is_login_configured():
    return bool(_SECRET_KEY_ENV and get_app_password_hash())
...
def get_favorites_listing(sort_key: str = "date", sort_dir: str = "desc"):
    ...
    if stale_paths:
        with favorites_state_lock:
            fresh_state = load_favorites_state()
            for rel_path in stale_paths:
                fresh_state["entries"].pop(rel_path, None)
            save_favorites_state(fresh_state)
...
def iter_xai_output_files():
    if not XAI_OUTPUT_DIR.exists() or not XAI_OUTPUT_DIR.is_dir():
        return
    allowed_extensions = {".png", ".jpg", ".jpeg", ".webp", ".mp4"}
    for entry in os.scandir(XAI_OUTPUT_DIR):
        ...
        yield {
            ...
            "stat": st,
            "is_video": is_video,
            "output_url": f"/api/xai/output/{entry.name}",
        }


def build_xai_output_item(item: dict) -> dict:
    thumb_sig = f"{int(item['stat'].st_mtime)}:{item['stat'].st_size}"
    return {
        "type": "xai_output",
        "name": item["name"],
        "rel_path": f"{XAI_DIR_KEY}/{item['name']}",
        "size_bytes": item["stat"].st_size,
        "mtime": int(item["stat"].st_mtime),
        "is_video": item["is_video"],
        "output_url": item["output_url"],
        "thumb_url": f"/api/xai/thumb/{item['name']}?mode=square&v={thumb_sig}",
        "thumb_sig": thumb_sig,
        "image_edit": None,
    }


def get_xai_listing(sort_key: str = "date", sort_dir: str = "desc"):
    items = [build_xai_output_item(f) for f in iter_xai_output_files()]

    if sort_key == "name":
        items.sort(key=lambda x: x["name"].lower(), reverse=(sort_dir == "desc"))
    else:
        items.sort(key=lambda x: x["mtime"], reverse=(sort_dir == "desc"))

    breadcrumb = [
        {"name": "root", "rel_path": ""},
        {"name": "xAI Outputs", "rel_path": XAI_DIR_KEY},
    ]
    return items, breadcrumb


def build_xai_folder_item():
    latest_mtime = 0
    count = 0
    for item in iter_xai_output_files():
        count += 1
        latest_mtime = max(latest_mtime, int(item["stat"].st_mtime))
    return {
        "type": "xai",
        "name": "xAI Outputs",
        "rel_path": XAI_DIR_KEY,
        "mtime": latest_mtime,
        "count": count,
    }


def generate_xai_thumbnail(file_path: Path, mode: str = "square") -> Path:
    """Generate a cached thumbnail for an xAI output file (image or video)."""
    if mode not in {"square", "full"}:
        raise ValueError("Unsupported thumbnail mode")

    st = file_path.stat()
    key = f"xai:{file_path.name}:{int(st.st_mtime)}:{st.st_size}:{mode}"
    digest = sha256(key.encode("utf-8")).hexdigest()
    ...
    if is_video:
        tmp_frame = cache_path.with_suffix(".tmp.png")
        try:
            subprocess.run(
                [
                    "ffmpeg", "-y",
        ...
        source_for_pil = file_path

    try:
        with Image.open(source_for_pil) as img:
...
def build_metadata_response(path: Path, parsed: dict, st=None):
    ...
    return {
        ...
        "edited_download_url": url_for("download_original", file=rel_path, variant="edited"),
        "workflow_download_url": url_for("download_workflow", file=rel_path),
...
def find_folder_cover(dir_path: Path, dir_stat=None):
    ...
    return find_newest_direct_png_cover(dir_path, dir_stat=st_dir)
...
def list_directory(rel_dir: str, sort_key: str = "date", sort_dir: str = "desc", *, include_thumb_dimensions: bool = False, include_folder_covers: bool = False):
    ...
    if rel_dir == FAVORITES_DIR_KEY:
        ...
        perf["png_count"] = len(png_items)
        return BROWSE_ROOT, [], png_items, breadcrumb, "", perf
    if rel_dir == XAI_DIR_KEY:
        xai_items, breadcrumb = get_xai_listing(sort_key, sort_dir)
        perf["png_count"] = len(xai_items)
        return BROWSE_ROOT, [], xai_items, breadcrumb, "", perf
    ...
    if current == BROWSE_ROOT:
        ...
        folders.insert(0, build_xai_folder_item())
...
def build_index_view_model(rel_dir: str, view_mode: str, sort_key: str, sort_dir: str):
    error = None
    is_virtual_dir = rel_dir in {FAVORITES_DIR_KEY, EDITS_DIR_KEY, XAI_DIR_KEY}
    timings = {
        "list_directory_ms": 0,
...
def index():
    ...
    if view_mode not in {"list", "thumbs", "thumbs-full"}:
...
def api_metadata_index_search():
    ...
    if view_mode not in {"list", "thumbs", "thumbs-full"}:
        view_mode = "thumbs-full"
    thumb_mode = "square" if view_mode == "thumbs" else "full"
...
def api_list_folders():
    ensure_root_exists()
    parent = (request.args.get("dir") or "").strip()
    try:
        parent_path = resolve_safe_path(parent)
    except ValueError as exc:
    ...
    try:
        ...
        return {"ok": False, "error": "Cannot read directory"}, 500
    folders.sort(key=lambda n: n.lower())
    parent_rel = rel_from_root(parent_path) if parent_path != BROWSE_ROOT else ""
    return {
        "ok": True,
        "dir": parent_rel,
        "folders": folders,
...
def api_create_folder():
    ...
    if not name:
        return {"ok": False, "error": "Folder name is required"}, 400
    if "/" in name or "\\" in name or name in {".", ".."}:
        return {"ok": False, "error": "Invalid folder name"}, 400
    try:
        parent_path = resolve_safe_path(parent)
    except ValueError as exc:
        return {"ok": False, "error": str(exc)}, 400
    if not parent_path.exists() or not parent_path.is_dir():
        return {"ok": False, "error": "Parent directory not found"}, 404
    new_path = parent_path / name
    if new_path.exists():
        return {"ok": False, "error": f"'{name}' already exists"}, 409
    try:
        new_path.mkdir(parents=False, exist_ok=False)
    except OSError as exc:
        return {"ok": False, "error": str(exc)}, 500
    new_rel = rel_from_root(new_path)
    log_event("folder_created", f"Created folder '{name}'", parent=parent or "root", folder=new_rel)
    return {"ok": True, "folder": new_rel, "name": name}
...
def download_original():
    ...
    if variant == "edited":
        effective = get_effective_image_path(rel_file, path)
        if effective == path:
            abort(404)
...
def api_upload():
    ...
    try:
        base_path = resolve_safe_path(target_dir)
    except ValueError as exc:
        return {"error": str(exc)}, 400
    ...
    for i, f in enumerate(files):
        rel_file_path = paths[i] if i < len(paths) else f.filename
        ...
        if ext not in ALLOWED_UPLOAD_EXTENSIONS:
            skipped += 1
            errors.append(f"Skipped (not an image): {rel_file_path}")
            continue

        try:
            dest = resolve_safe_path(
                str(Path(target_dir) / rel_file_path) if target_dir else rel_file_path
            )
        except ValueError:
            skipped += 1
            errors.append(f"Skipped (invalid path): {rel_file_path}")
            continue
        ...
        try:
            ...
            skipped += 1
            errors.append(f"Failed to save {rel_file_path}: {exc}")

    log_event("upload_complete", f"{saved} saved, {skipped} skipped")
    return {"saved": saved, "skipped": skipped, "errors": errors}


@app.route("/download-workflow")
def download_workflow():
    ...
    try:
        parsed = PNGMetadataParser().parse_png(path)
    except Exception:
        abort(500)
    ...
    download_name = path.stem + "_workflow.json"
    return Response(
        workflow_json,
...
def _clamp_float(value, min_value: float, max_value: float, default: float) -> float:
    try:
        numeric = float(value) if value is not None else float(default)
    except (TypeError, ValueError):
        numeric = float(default)
    return max(min_value, min(max_value, numeric))


@app.route("/api/image-edits/phone-look", methods=["POST"])
def api_phone_look_image():
    ensure_root_exists()
    payload = request.get_json(silent=True) or {}
    rel_file = str(payload.get("file") or "").strip()
    if not rel_file:
    ...
    if not path.exists() or not path.is_file() or path.suffix.lower() != ".png":
        return {"ok": False, "error": "Not a valid PNG file"}, 400
    if np is None:
        return {"ok": False, "error": "Phone Look dependencies are not installed"}, 409

    shadow_lift = _clamp_float(payload.get("shadow_lift"), 0.0, 0.5, 0.15)
    highlight_compress = _clamp_float(payload.get("highlight_compress"), 0.0, 0.5, 0.10)
    local_tone_map = _clamp_float(payload.get("local_tone_map"), 0.0, 1.0, 0.40)
    local_radius_pct = _clamp_float(payload.get("local_radius_pct"), 1.0, 25.0, 8.0)
    saturation_boost = _clamp_float(payload.get("saturation_boost"), 0.5, 2.0, 1.15)
    warmth = _clamp_float(payload.get("warmth"), -0.10, 0.10, 0.02)
    ...
    try:
        ...
        with Image.open(effective_path) as img:
            source_image = ImageOps.exif_transpose(img).convert("RGBA")
        output = sensor_realism.apply_phone_look(
            source_image,
            shadow_lift=shadow_lift,
            highlight_compress=highlight_compress,
            local_tone_map=local_tone_map,
            local_radius_pct=local_radius_pct,
        ...
        log_event("image_edit_phone_look_failure", "Phone Look save failed", file=rel_file, error=str(exc))
        return {"ok": False, "error": f"Failed to apply Phone Look: {exc}"}, 500

    log_event(
        "image_edit_phone_look_completed",
        "Phone Look applied",
        file=rel_file,
        shadow_lift=shadow_lift,
        highlight_compress=highlight_compress,
...
def api_delete_files():
    ...
    if not isinstance(files, list) or not files:
        return {"ok": False, "error": "Missing 'files' array"}, 400
    if len(files) > 500:
        return {"ok": False, "error": "Too many files (max 500)"}, 400
    ...
    for rel_file in files:
        ...
        try:
            ...
            log_event("file_delete_failure", "Delete rejected: invalid path", file=rel_file, error=str(exc))
            failed.append({"file": rel_file, "error": str(exc)})
            continue
        if not path.exists():
            log_event("file_delete_failure", "Delete rejected: not found", file=rel_file)
            failed.append({"file": rel_file, "error": "Not found"})
            continue
        if path == BROWSE_ROOT:
            log_event("file_delete_failure", "Delete rejected: cannot delete root", file=rel_file)
            failed.append({"file": rel_file, "error": "Cannot delete root"})
            continue
        is_dir = path.is_dir()
        try:
            if is_dir:
            ...
            log_event(
                "file_delete_failure",
                "Delete failed: filesystem error",
                file=rel_file,
        ...
        if not is_dir:
            clear_image_edit(rel_file)
            with favorites_state_lock:
                fav_state = load_favorites_state()
                if rel_file in fav_state["entries"]:
                    fav_state["entries"].pop(rel_file, None)
                    save_favorites_state(fav_state)
            with metadata_cache_lock:
                keys_to_remove = [k for k in metadata_cache if k[0] == str(path)]
    ...
    if deleted:
        try:
            ...
            try:
                metadata_index.initialize(conn)
                metadata_index.delete_missing_files(conn, deleted)
                conn.commit()
            finally:
                conn.close()
        except Exception:
    ...
    log_event("files_deleted", f"Deleted {len(deleted)} file(s)", deleted_count=len(deleted), failed_count=len(failed))
    return {
        "ok": True,
        "deleted": deleted,
...
def api_move_files():
    ...
    if not isinstance(items, list) or not items:
        return {"ok": False, "error": "Missing 'items' array"}, 400
    if len(items) > 500:
        return {"ok": False, "error": "Too many items (max 500)"}, 400

    try:
        dest_path = resolve_safe_path(destination)
    except ValueError as exc:
        return {"ok": False, "error": f"Invalid destination: {exc}"}, 400
    if not dest_path.exists() or not dest_path.is_dir():
        return {"ok": False, "error": "Destination folder not found"}, 404

    moved = []
    failed = []
    for rel_item in items:
        rel_item = str(rel_item or "").strip()
        if not rel_item:
            continue
        try:
            source = resolve_safe_path(rel_item)
        except ValueError as exc:
            failed.append({"item": rel_item, "error": str(exc)})
            continue
        if not source.exists():
            failed.append({"item": rel_item, "error": "Not found"})
            continue
        ...
        if new_path.exists():
            ...
            counter = 2
            while new_path.exists() and counter < 10000:
                new_path = dest_path / f"{stem} ({counter}){suffix}"
                counter += 1
            if new_path.exists():
                failed.append({"item": rel_item, "error": f"Cannot find unique name for '{source.name}'"})
                continue
        is_dir = source.is_dir()
        if is_dir:
            try:
                new_path.relative_to(source)
                failed.append({"item": rel_item, "error": "Cannot move a folder into itself"})
                continue
            except ValueError:
                pass
        try:
            ...
            try:
                ...
                failed.append({"item": rel_item, "error": str(exc2)})
                continue
        old_rel = rel_item
        new_rel = rel_from_root(new_path)

        if not is_dir:
            clear_image_edit(old_rel)
            with favorites_state_lock:
                fav_state = load_favorites_state()
                if old_rel in fav_state["entries"]:
                    fav_state["entries"].pop(old_rel, None)
                    save_favorites_state(fav_state)
            with metadata_cache_lock:
                keys_to_remove = [k for k in metadata_cache if k[0] == str(source)]
                for k in keys_to_remove:
                    metadata_cache.pop(k, None)
    ...
    if moved:
        ...
        try:
            ...
            try:
                metadata_index.initialize(conn)
                metadata_index.delete_missing_files(conn, old_paths)
                conn.commit()
            finally:
                conn.close()
        except Exception:
    ...
    log_event("files_moved", f"Moved {len(moved)} item(s)", moved_count=len(moved), failed_count=len(failed), destination=destination)
    return {
        "ok": True,
        "moved": moved,
...
def xai_is_configured():
    return XAI_ENABLED and bool((os.getenv("XAI_API_KEY") or "").strip().lstrip("="))


@app.route("/api/xai/image-edit", methods=["POST"])
def api_xai_image_edit():
    ensure_root_exists()
    if not xai_is_configured():
        return {"ok": False, "error": "xAI is not configured. Set XAI_API_KEY in your .env file."}, 409
    ...
    if not rel_file:
        return {"ok": False, "error": "Missing 'file' parameter"}, 400
    if not prompt:
        return {"ok": False, "error": "Missing 'prompt' parameter"}, 400
    ...
    log_event("xai_image_edit_started", "xAI image edit started", file=rel_file, prompt=prompt[:200])

    try:
        result = xai_generation.edit_image(path, prompt, quality=quality, resolution=resolution)
    except xai_generation.XAIGenerationError as exc:
        log_event("xai_image_edit_failed", "xAI image edit failed", file=rel_file, error=str(exc))
        return {"ok": False, "error": str(exc)}, 500

    safe_stem = "".join(c if c.isalnum() or c in "-_" else "_" for c in path.stem)[:60]
    output_name = f"xai_edit_{int(time.time())}_{safe_stem}.png"
    output_path = XAI_OUTPUT_DIR / output_name
    ...
    try:
        if b64_data:
            xai_generation.save_edited_image(b64_data, output_path)
        elif remote_url:
            xai_generation.download_video(remote_url, output_path)
        else:
            return {"ok": False, "error": "xAI response contained no image data"}, 500
    except Exception as exc:
        log_event("xai_image_edit_save_failed", "Failed to save xAI edited image", file=rel_file, error=str(exc))
        return {"ok": False, "error": f"Failed to save result: {exc}"}, 500

    log_event("xai_image_edit_completed", "xAI image edit completed", file=rel_file, output=output_name)
    return {
        "ok": True,
        "file": rel_file,
        "output_file": output_name,
        "output_url": f"/api/xai/output/{output_name}",
        "revised_prompt": result.get("revised_prompt", ""),
    }


@app.route("/api/xai/video-generate", methods=["POST"])
def api_xai_video_generate():
    ensure_root_exists()
    if not xai_is_configured():
        return {"ok": False, "error": "xAI is not configured. Set XAI_API_KEY in your .env file."}, 409
    ...
    log_event("xai_video_generate_started", "xAI video generation started", file=rel_file, prompt=prompt[:200])

    try:
        request_id = xai_generation.generate_video(path, prompt, duration=duration, resolution=resolution)
    except xai_generation.XAIGenerationError as exc:
        log_event("xai_video_generate_failed", "xAI video generation failed", file=rel_file, error=str(exc))
        return {"ok": False, "error": str(exc)}, 500

    with xai_video_jobs_lock:
    ...
    log_event("xai_video_generate_submitted", "xAI video generation submitted", file=rel_file, request_id=request_id)
    return {
        "ok": True,
        "file": rel_file,
...
@app.route("/api/xai/video-status/<request_id>")
def api_xai_video_status(request_id):
    if not xai_is_configured():
        return {"ok": False, "error": "xAI is not configured"}, 409

    try:
        status_info = xai_generation.poll_video_status(request_id)
    except xai_generation.XAIGenerationError as exc:
        return {"ok": False, "error": str(exc)}, 500
    ...
    if status == "done" and video_url:
        output_name = f"xai_video_{int(time.time())}_{request_id[:8]}.mp4"
        output_path = XAI_OUTPUT_DIR / output_name
        try:
            xai_generation.download_video(video_url, output_path)
            log_event("xai_video_download_completed", "xAI video downloaded", request_id=request_id, output=output_name)
        except Exception as exc:
            log_event("xai_video_download_failed", "Failed to download xAI video", request_id=request_id, error=str(exc))
            return {"ok": False, "error": f"Video ready but download failed: {exc}"}, 500

        with xai_video_jobs_lock:
            xai_video_jobs.pop(request_id, None)
...
@app.route("/api/xai/output/<filename>")
def api_xai_output(filename):
    filename = str(filename).strip()
    if not filename or "/" in filename or "\\" in filename or ".." in filename:
        abort(400)

    file_path = (XAI_OUTPUT_DIR / filename).resolve()
    try:
        file_path.relative_to(XAI_OUTPUT_DIR)
    except ValueError:
        abort(400)
...
@app.route("/api/xai/thumb/<filename>")
def api_xai_thumb(filename):
    filename = str(filename).strip()
    if not filename or "/" in filename or "\\" in filename or ".." in filename:
        abort(400)
    ...
    mode = request.args.get("mode", "square").lower()
    if mode not in {"square", "full"}:
        abort(400)

    try:
        cache_path = generate_xai_thumbnail(file_path, mode=mode)
    except Exception:
        abort(500)
...
def api_xai_status():
    return {"ok": True, "configured": xai_is_configured()}
